...
async function buildCanvas(d: DesignData, p1src: string | null, p2src: string | null): Promise<HTMLCanvasElement> {
  const W = 1080, H = 1920
  ...
  if (i1) {
    ...
    ctx.drawImage(i1, (W - i1.width * s) / 2, (HERO - i1.height * s) / 2, i1.width * s, i1.height * s)
    const g = ctx.createLinearGradient(0, 0, 0, HERO)
    g.addColorStop(0, 'rgba(26,38,38,0.55)')
    g.addColorStop(0.30, 'rgba(26,38,38,0.02)')
  ...
  // Location
  ctx.beginPath(); ctx.arc(62, HERO - 208, 8, 0, Math.PI * 2)
  ...
  ctx.font = `300 105px ${SF}`; ctx.fillStyle = WHITE; ctx.textAlign = 'left'
  ctx.fillText(words[0] || '', 52, HERO - 95)
  if (words.length > 1) {
    const w0 = ctx.measureText((words[0] || '') + ' ').width
    ctx.font = `bold 105px ${SF}`; ctx.fillStyle = '#D4EDEF'
  ...
  // Stats card
  const SY = IY + 214, SW = W - 104, SH = 162
  ctx.fillStyle = WHITE
  ...
  ;([
    ...
    ctx.font = `bold 68px ${SF}`; ctx.fillStyle = TEAL; ctx.textAlign = 'center'
  ...
  if (i2) {
    ...
    ctx.drawImage(i2, 52 + (P2W - i2.width * s2) / 2, P2Y + (P2H - i2.height * s2) / 2, i2.width * s2, i2.height * s2)
    ctx.restore()
  } else {
    ctx.fillStyle = PH_C; rr(ctx, 52, P2Y, P2W, P2H, 20); ctx.fill()
  }
  ctx.strokeStyle = 'rgba(61,138,143,0.18)'; ctx.lineWidth = 2
  rr(ctx, 52, P2Y, P2W, P2H, 20); ctx.stroke()
...
export default function StoryEditor() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#091212', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 16px 64px', fontFamily: SS }}>
      ...
      {/* Story card */}
      <div style={{ width: PW, height: PH, borderRadius: 16, overflow: 'hidden', background: BG, display: 'flex', flexDirection: 'column', flexShrink: 0, boxShadow: '0 0 0 1px rgba(61,138,143,0.35), 0 40px 80px rgba(0,0,0,0.85)' }}>

        {/* Hero */}
        <div style={{ height: HPX, position: 'relative', flexShrink: 0 }}>
          <Photo src={p1} onLoad={setP1} label="Klicka för foto 1" sub="Fastighetens exteriör" style={{ position: 'absolute', inset: 0, zIndex: 0 }} />
          <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none', background: heroGradient }} />

          {/* Badge + REF */}
          <div style={{ position: 'absolute', top: 10, left: 10, right: 10, zIndex: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center', pointerEvents: 'none' }}>
            ...
            <div style={{ pointerEvents: 'auto' }}>
              <Editable value={d.ref} onChange={set('ref')} style={{ fontSize: 7, color: 'rgba(255,255,255,0.55)', letterSpacing: 2, fontFamily: SS }} />
            </div>
          </div>

          {/* Location + Title */}
          <div style={{ position: 'absolute', bottom: 8, left: 11, right: 11, zIndex: 4, pointerEvents: 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2, pointerEvents: 'auto' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: TEAL, flexShrink: 0 }} />
              <Editable value={d.location} onChange={set('location')} style={{ fontSize: 8, color: 'rgba(255,255,255,0.78)', letterSpacing: 1, fontFamily: SS, fontWeight: 300 }} />
            </div>
            <div style={{ pointerEvents: 'auto' }}>
              <Editable value={d.title} onChange={set('title')} style={{ fontFamily: SF, fontSize: 22, fontWeight: 300, color: WHITE, lineHeight: 1.05 }} />
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, background: BG, padding: '7px 11px 4px', display: 'flex', flexDirection: 'column', gap: 5, overflow: 'hidden' }}>
          ...
          {/* Stats */}
          <div style={{ background: WHITE, borderRadius: 9, padding: '5px 0', display: 'grid', gridTemplateColumns: '1fr 1.5px 1fr 1.5px 1fr', boxShadow: '0 2px 14px rgba(61,138,143,0.07)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, padding: '3px 0' }}>
              <BedIcon />
              <Editable value={d.beds} onChange={set('beds')} center style={{ fontFamily: SF, fontSize: 15, fontWeight: 'bold', color: DARK, textAlign: 'center' }} />
              <div style={{ fontSize: 5, color: MUTED, letterSpacing: 2, textTransform: 'uppercase', fontFamily: SS }}>Beds</div>
            </div>
            <div style={{ background: 'rgba(61,138,143,0.13)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, padding: '3px 0' }}>
              <BathIcon />
              <Editable value={d.baths} onChange={set('baths')} center style={{ fontFamily: SF, fontSize: 15, fontWeight: 'bold', color: DARK, textAlign: 'center' }} />
              <div style={{ fontSize: 5, color: MUTED, letterSpacing: 2, textTransform: 'uppercase', fontFamily: SS }}>Baths</div>
            </div>
            <div style={{ background: 'rgba(61,138,143,0.13)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, padding: '3px 0' }}>
              <AreaIcon />
          ...
          {/* Photo 2 */}
          <Photo src={p2} onLoad={setP2} label="Klicka för foto 2" sub="Pool / trädgård / interiör" style={{ flex: 1, minHeight: 55, borderRadius: 7, border: '1.5px solid rgba(61,138,143,0.20)' }} />

          {/* Footer */}
          <div style={{ textAlign: 'center', paddingBottom: 2 }}>
            <Editable value={d.tagline} onChange={set('tagline')} center style={{ fontSize: 6, color: MUTED, letterSpacing: 2, fontFamily: SS }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 8, color: '#162020', fontSize: 9, letterSpacing: 2, fontFamily: SS }}>PNG · 1080 × 1920 px</div>
    </div>
  )
}
