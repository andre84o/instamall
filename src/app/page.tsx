...
export default function Home() {
  const [active, setActive] = useState<'story' | 'property'>('story')
  ...
  return (
    <div style={{ background: '#091212', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: SS }}>
      ...
      <div style={{ display: 'flex', gap: 2, marginTop: 28, marginBottom: 0 }}>
        {(['story', 'property'] as const).map(tab => (
          <button
            key={tab}
          ...
          >
            {tab === 'story' ? 'Story' : 'Property'}
          </button>
        ))}
      </div>

      {active === 'story' ? <StoryEditor /> : <PropertyStoryEditor />}
    </div>
  )
}
